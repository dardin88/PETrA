...
public class ICSEExperimentMain {
    ...
    public static void main(String[] args) throws InterruptedException, ParseException {
        ...
        for (int appCounter = 0; appCounter < appNames.size(); appCounter++) {
            ...
            try {
                ...
                Logger.getLogger(ICSEExperimentMain.class.getName()).log(Level.SEVERE, null, ex);
            }
            ...
            ICSEExperimentMain.executeCommand("adb shell dumpsys battery set usb 0", null, null, true);

            System.out.println("Installing app.");
            ICSEExperimentMain.executeCommand("adb install " + apkLocation, null, null, true);
            ...
            for (int run = 0; run < maxRun; run++) {
                ...
                ICSEExperimentMain.executeCommand("adb shell pm clear " + appName, null, null, true);

                System.out.println("Resetting battery stats.");
                ICSEExperimentMain.executeCommand("adb shell dumpsys batterystats --reset", null, null, true);

                System.out.println("Opening app.");
                ICSEExperimentMain.executeCommand("adb shell input keyevent 82", null, null, true);
                ICSEExperimentMain.executeCommand("adb shell monkey -p " + appName + " 1", null, null, true);
                ICSEExperimentMain.executeCommand("adb shell am broadcast -a org.thisisafactory.simiasque.SET_OVERLAY --ez enable true", null, null, true);

                System.out.println("Start profiling.");
                ICSEExperimentMain.executeCommand("adb shell am profile start " + appName + " ./data/local/tmp/log.trace", null, null, true);
                Date time1 = new Date();
                ...
                ICSEExperimentMain.executeCommand("adb kill-server", null, null, true);
                ICSEExperimentMain.executeCommand("adb start-server", null, null, true);
                ICSEExperimentMain.executeCommand("adb shell monkey -p " + appName + " -s " + seed + " --throttle " + timeBetweenInteractions + " --ignore-crashes --ignore-timeouts --ignore-security-exceptions " + interactions, null, null, true);
                ...
                ICSEExperimentMain.executeCommand("adb shell am profile stop " + appName, null, null, true);

                System.out.println("Saving battery stats.");
                ICSEExperimentMain.executeCommand("adb shell dumpsys batterystats", null, new File(batteryStatsFilename), true);

                System.out.println("Saving traceviews.");
                ICSEExperimentMain.executeCommand("adb pull ./data/local/tmp/log.trace " + runDataFolderName, null, null, true);
                ICSEExperimentMain.executeCommand("./dmtracedump -o " + runDataFolderName + "log.trace", platformToolsFolder, new File(traceviewFilename), true);
                ...
                try {
                    ...
                    try (PrintWriter resultsWriter = new PrintWriter(runDataFolderName + "result.csv", "UTF-8")) {
                        ...
                        energyInfoArray = ICSEExperimentMain.mergeEnergyInfo(energyInfoArray, cpuInfo, powerProfile);
                        for (TraceLine traceLine : traceLines) {
                            List<Double> result = ICSEExperimentMain.calculateConsumption(traceLine.getEntrance(), traceLine.getExit(), energyInfoArray, powerProfile);
                    ...
                    ICSEExperimentMain.executeCommand("adb shell am broadcast -a org.thisisafactory.simiasque.SET_OVERLAY --ez enable false", null, null, true);
                    ICSEExperimentMain.executeCommand("adb shell am force-stop " + appName, null, null, true);
            ...
            ICSEExperimentMain.executeCommand("adb shell dumpsys battery reset", null, null, true);

            System.out.println("Uninstalling app.");
            ICSEExperimentMain.executeCommand("adb shell pm uninstall " + appName, null, null, true);
        }
    ...
    private static void executeCommand(String command, File directoryFolder, File outputFile, boolean waitfor) {
        try {
            ...
            Logger.getLogger(ICSEExperimentMain.class
